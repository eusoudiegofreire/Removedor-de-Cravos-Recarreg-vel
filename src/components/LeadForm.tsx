...
export function LeadForm() {
  const [isOpen, setIsOpen] = useState(
    () => typeof window !== "undefined" && window.location.hash === `#${FORM_ANCHOR}`,
  );
  ...
  useEffect(() => {
    // next/link updates the URL through the History API instead of a real
    // anchor navigation, so it never fires a native "hashchange" event —
    // catch the click directly instead of relying on the hash to change.
    function openOnAnchorClick(event: MouseEvent) {
      const target = event.target as HTMLElement;
      const anchor = target.closest?.(`a[href="#${FORM_ANCHOR}"]`);
      if (anchor) setIsOpen(true);
    }

    document.addEventListener("click", openOnAnchorClick);
    return () => document.removeEventListener("click", openOnAnchorClick);
  }, []);
  ...
  return (
    <>
      <div
        id={FORM_ANCHOR}
        className="reveal mx-auto max-w-xl rounded-card border border-border bg-white p-6 text-center shadow-sm sm:p-8"
      >
        ...
        <h3 className="mt-4 font-heading text-xl font-bold text-text">Faça seu agendamento agora</h3>
        <p className="mt-3 text-sm leading-relaxed text-text-secondary">
          Preencha seus dados em poucos passos. Confirmamos tudo pelo WhatsApp antes do envio.
        </p>
        <button
          type="button"
      ...
      {isOpen ? (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-text/55 px-4 py-6"
        ...
        >
          <div
            ...
            className="relative flex max-h-[90vh] w-full max-w-md flex-col overflow-y-auto rounded-card bg-white p-6 shadow-xl transition-[height] duration-300 ease-snappy sm:p-8"
          >
            <button
              type="button"
              onClick={() => setIsOpen(false)}
